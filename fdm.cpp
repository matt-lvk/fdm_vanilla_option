...
void FDMEulerExplicit::calculate_boundary_conditions() {
  new_result[0] = pde->boundary_left(x_values[0], prev_t);
  new_result[J-1] = pde->boundary_right(x_values[J-1], prev_t);
}
...
void FDMEulerExplicit::calculate_inner_domain() {
  ...
  for (unsigned long j=1; j<J-1; j++) {
    ...
    double dt_sig = dt * (pde->diff_coeff(x_values[j], prev_t));
    double dt_sig_2 = dt * dx * 0.5 * (pde->conv_coeff(x_values[j], prev_t));

    // Differencing coefficients (see \alpha, \beta and \gamma in text)
    alpha = dt_sig - dt_sig_2;
    beta = dx * dx - (2.0 * dt_sig) + (dt * dx * dx * (pde->zero_coeff(x_values[j], prev_t)));
    ...
    new_result[j] = ( (alpha * old_result[j-1]) +
      ...
      (dt*(pde->source_coeff(x_values[j], prev_t)));
  }
}
